...
# Tag the monthly backup (1st of each month) by keeping it
# Tag weekly backups (every Monday) by keeping them
# All others are daily — keep only the 3 most recent
...
for f in ${BACKUPS}; do
    ...
    if [ "$FILE_DAY_OF_MONTH" = "01" ]; then
        # Monthly backup (1st of month) — keep 1 most recent
        ...
        # Weekly backup (Monday) — keep 2 most recent
        ...
        # Daily backup — already handled by the loop above
        :
    fi
done

# For daily backups (non-Monday, non-1st), keep only 3 most recent
DAILY_COUNT=0
for f in $(ls -1t "${BACKUP_DIR}"/backup-*.tar.gz 2>/dev/null || true); do
    ...
    # Skip monthly and weekly backups
